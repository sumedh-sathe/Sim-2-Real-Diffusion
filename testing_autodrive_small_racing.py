...
try:
    ...
    pipe = AutoPipelineForImage2Image.from_pretrained(
        "../training/model_autodrive_small_racing",
        scheduler = DPMSolverMultistepScheduler.from_pretrained("../training/model_autodrive_small_racing", subfolder="scheduler"),
...
image_folder = os.path.join(script_dir, "input_autodrive_small_racing") # Load input frames from here
style_folder = os.path.join(script_dir, "style_autodrive_small_racing") # Load style frames from here
output_folder = os.path.join(script_dir, "output_autodrive_small_racing") # Store output results here
...
for i in range(len(image_files)):
    ...
    output = pipe(
        prompt="<autodrive_small_racing> simulation",
        ...
        strength=0.2, # Adherence to input image (higher values = more freedom to change, lower values = more faithful)
        num_inference_steps=10, # Denoising steps (e.g., 5 effectively gets converted to 3)
        guidance_scale=4.0 # Adherence to prompt (higher values = more prompt adherence, lower values = more creativity)
    ).images[0]
